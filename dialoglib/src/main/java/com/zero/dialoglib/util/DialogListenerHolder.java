...
public class DialogListenerHolder {
    ...
    private static final String EXTRA_LISTENER_KEY = "listener_key";
    ...
    /**
     * 把listener的key值保存在bundle中，配置发生变化的情况下（横屏换为竖屏），
     * 在从bundle中取listener的key值
     *
     * @param outState
     */
    public void saveDialogListenerKey(Bundle outState) {
        if (outState != null) {
            outState.putString(EXTRA_LISTENER_KEY, mDialogListenerKey);
        }
    }
    ...
    public void getDialogListenerKey(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            mDialogListenerKey = savedInstanceState.getString(EXTRA_LISTENER_KEY);
        }
    }
    ...
    private boolean isNeedRestoreDialogListener() {
        return mDialogListenerKey == null ? false : true;
    }
}
